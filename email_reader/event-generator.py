...
from messages_pb2 import GreetRequest, GreetResponse, Email
...
def consume_email():
    consumer = KafkaConsumer(
        ...
        bootstrap_servers=[KAFKA_BROKER],
        auto_offset_reset='earliest',
        group_id='email_reqader')
    for message in consumer:
        response = Email()
        response.ParseFromString(message.value)
        print("SENT EMAIL TO !!!!!!!!!!!: %s" % (response.recipient), flush=True)
...
def main():
    ...
    email_consumer = threading.Thread(target=safe_loop, args=[consume_email])
